...
function XeetGrid() {
  ...
  return (
    <>
      {
        xeets.map((xeet) => {
          return <XeetComponent key={xeet.user} xeet={xeet} />
        })
      }
    </>
  )
...
function XeetComponent({ xeet }: {
  xeet: Record<string, string>
}) {
  // probably just do a tweet embed like from x.com 
  return (
    <div className="xeet-component" >
      <div>
        <img src={img} />
